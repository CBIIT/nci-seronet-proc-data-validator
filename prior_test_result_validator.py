...
#################################################################################################################################################
        if header_name.find('Research_Participant_ID') > -1:        #checks if Participant ID in valid format
            ...
            for i in enumerate(has_data_column):
                prior_valid_object.valid_ID(header_name,i[1],pattern,valid_cbc_ids,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                prior_valid_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
...
#################################################################################################################################################
            ...
            for i in enumerate(has_data_column):
                prior_valid_object.is_date_time(header_name,i[1],False,error_msg,has_data_column.index[i[0]],'Error') 
            for i in enumerate(missing_data_column):
                prior_valid_object.is_required(header_name,i[1],"ALL",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
            ...
            for i in enumerate(has_data_column):
                prior_valid_object.is_date_time(header_name,i[1],True,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                prior_valid_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
        elif (header_name.find('SARS_CoV_2_PCR_Test_Result_Provenance') > -1):
            test_string = ['Self-reported','From Medical Record','Self-Reported']
            error_msg = "Value must be one of the following: " + str(test_string)
            for i in enumerate(has_data_column):
                prior_valid_object.in_list(header_name,i[1],test_string,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                prior_valid_object.is_required(header_name,i[1],"ALL",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
...
#################################################################################################################################################
            ...
            test_string = ['Self-Reported','Self-reported','From Medical Record','N/A']
            ...
            for i in enumerate(pos_test_value):
                prior_valid_object.in_list(header_name,i[1],test_string,error_msg,pos_test_value.index[i[0]],'Error') 
            ...
            for i in enumerate(neg_test_value):
                prior_valid_object.in_list(header_name,i[1],test_string,error_msg,neg_test_value.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                prior_valid_object.is_required(header_name,i[1],"ALL",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
            ...
            for i in enumerate(test_value):
                prior_valid_object.is_numeric(header_name,False,i[1],0,error_msg,test_value.index[i[0]],'Error')
            ...
            for i in enumerate(has_data_column):
                prior_valid_object.in_list(header_name,i[1],["N/A"],error_msg,has_data_column.index[i[0]],'Error')
            ...
            for i in enumerate(has_data_column):
                prior_valid_object.in_list(header_name,i[1],[''],error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                prior_valid_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Warning')
#################################################################################################################################################
...
#################################################################################################################################################
            ...
            for i in enumerate(pos_test_value):
                prior_valid_object.in_list(header_name,i[1],test_string,error_msg,pos_test_value.index[i[0]],'Error')
            test_string = ['Positive','Negative','Equivocal','Not Performed']
            error_msg = "Participant is SARS_CoV2 Negative. Value must be: " + str(test_string)
            neg_test_value = prior_valid_object.Data_Table[prior_valid_object.neg_list_logic & has_logic][header_name]
            for i in enumerate(neg_test_value):
                prior_valid_object.in_list(header_name,i[1],test_string,error_msg,neg_test_value.index[i[0]],'Error')
