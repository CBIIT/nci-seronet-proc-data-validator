    ...
    for header_name in data_object.Column_Header_List:
        ...
        if (header_name in ["Biospecimen_ID"]):
            ...
            for i in enumerate(has_data_column):
                data_object.valid_ID(header_name,i[1],pattern,valid_cbc_ids,error_msg,has_data_column.index[i[0]],'Error') 
            for i in enumerate(missing_data_column):
                data_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
...
#################################################################################################################################################
            ...
            for i in enumerate(has_data_column):
                data_object.is_string(header_name,i[1],False,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                data_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
            ...
            for i in enumerate(has_data_column):
                data_object.is_numeric(header_name,False,i[1],0,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                data_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
            ...
            for i in enumerate(has_data_column):
                data_object.is_date_time(header_name,i[1],False,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                data_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################
...
#################################################################################################################################################
            ...
            for i in enumerate(has_data_column):
                data_object.in_list(header_name,i[1],test_string,error_msg,has_data_column.index[i[0]],'Error')
            for i in enumerate(missing_data_column):
                data_object.is_required(header_name,i[1],"All",missing_data_column.index[i[0]],'Error')
#################################################################################################################################################      
    return data_object